...
flowchart TD
    ...
    RELEASE --> JUDGE{"게이지 판정에 따른\n성공 / 실패 여부"}

    %% 판정 분기
    JUDGE -->|"< 70%"| FAIL_LOW["실패"]
    JUDGE -->|"70 ~ 85%"| SUCCESS["성공\n(바람 불고 주입)"]
    JUDGE -->|"> 85%"| FAIL_HIGH["실패"]

    %% 실패 경로
    FAIL_LOW  --> RESET_LOW["게이지 리셋"]
    FAIL_HIGH --> RESET_HIGH["게이지 리셋"]

    %% 성공 경로
    SUCCESS --> WEB["거미줄 제거 진행\nwebLayerIndex --"]
    WEB --> INC["SuccessCount += 1"]
    INC --> CHECK{"SuccessCount = N ?"}

    %% 재시도 루프
    RESET_LOW  --> TC2_RETRY{"시간 초과?"}
    RESET_HIGH --> TC2_RETRY
    ...
    style FAIL_LOW      fill:#e74c3c,color:#fff
    style FAIL_HIGH     fill:#e74c3c,color:#fff
